...
func TestLess(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			fit := &Fit{}
			if got := fit.Less(tt.p1, tt.p2); got != tt.expected {
...
func TestPreFilter(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			fit := &Fit{podLister: podInformer.Lister()}
			for _, p := range tt.pods {
				fit.getOrCreatePodGroupInfo(p, time.Now())
				podInformer.Informer().GetStore().Add(p)
			}

			podInformer.Informer().GetStore().Add(tt.pod)
			cycleState := framework.NewCycleState()
			if got := fit.PreFilter(nil, cycleState, tt.pod); got.Code() != tt.expected {
